...
Ha = the soil of the burned and unburned sites do not have the same phosphorus content.
Statistical:
...
b. A t-test can be used to determine if the mean of the two areas is significantly different.
...
e. The null hypothesis was that there is no difference in phosphorus content between the burned and unburned sites. 34 soil samples were collected and there was found to be a significant difference in phosphorus content between burned and unburned sites (two-sample
two-tailed t-test, t(66) = 2.5, ⍺ = 0.05, p = 0.014)
...
g. I would preform a t-test to compare the mean phosphorus content of the soil before and after the fire in each survey site in order to determine if there is a significant difference in the means. 
...
Generally, flowers across a road or bike path are less likely to be visited by pollinators than flowers across a road or bike path. The pedestrian path seemed to have little effect on the rate of pollination.
...
veg <- read.csv("~/git/ENVS-193DS_midterm_miller_katie/data/roadveg.csv",
                skip=1, colClasses = c(mowed = "factor",
                                       flowers = "factor",
                                       seminatural = "factor", 
                                       weed = "factor"))
...
c. Using the Kruskal-Wallis test can help us determine if the median scat length is the same between the two habitat types. compare the median of two random data sets
This test is the best choice because it  whether the medians of two data sets.
...
f. The null hypothesis is that there is no significant difference in median coyote scat length between the grassland and shrubland sites in the Chihuahuan Desert. The null hypothesis is accepted
